...
func (email *Email) readAttachments() error {
	for _, file := range append(email.attachments, email.inlines...) {

		name, mimeType := file.Name, file.MimeType
		...
		attachTy, err := getAttachmentType(file)
		if err != nil {
			return errors.New("Mail Error: Failed to add attachment with following error: " + err.Error())
		}
		...
		case attachFile:
			dec, err = os.ReadFile(file.FilePath)
			if err != nil {
				return errors.New("Mail Error: Failed to add file with following error: " + err.Error())
			}
		}
